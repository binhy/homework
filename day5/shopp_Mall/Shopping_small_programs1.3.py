    ...
    while count < 3:
        if username in lock_user_list:    #判断用户是否被锁
            ...
            if username in account_dict:  #判断是否有这个用户
                passwd=input("请输入密码:")  #有这个用户，那么请输入密码
...
def budget():
    global init_balance
    while True:
        init_balance=input("购物之前，请输入您的预算金额: ")
...
def welcome():
    ...
    ###############################################
    ''')
    init_your_choice=input("please choice your like shop and do somthing:")
...
def shangpin(num,surplus_balance):
    if num in Item.keys():   #你选择的存在商品字典里
        if surplus_balance < 7000 or surplus_balance < 8000 or surplus_balance < 9000:
            ...
            compute_balance=surplus_balance - Item[num].values()  #计算余额
            if list(Item[num].keys())[0] in shopping_cart_dict:  #如果商品已经买过
                shopping_cart_dict[list(Item[num].keys())[0]][1]=shopping_cart_dict[list(Item[num].keys())[0]][1] + 1  #那么在购物车里的这个商品数量+1
                return compute_balance  #返回一个计算过的余额
            else:                           #如果商品没有买过,初始化购物车这个商品数量为1
                init_num=1
                shopping_cart_dict[list(Item[num].keys())[0]]=[list(Item[num].values())[0],init_num]
                return compute_balance #返回一个计算过的余额
    else:  #你选择的不存在商品字典
...
def recharge(surplus_balance):
    global now_balance
    while True:
        recharge_balance=input("please your choice recharge balance:")
...
def neaten_shopping_cart(surplus_blance):
    ...
    while True: #第一层循环
        ...
        product_id=input("请选择商品编号（1:iphone7, 2:ipad_air2, 3:ipad_pro），或者q退出:\n")
        if product_id == "q":  #输入q，退出第一层循环
            break
        if len(product_id) ==0:
            print("输入不能为空，请重新输入！\n")
            continue
        if str.isalpha(product_id) or int(product_id) not in Item.keys() or list(Item[int(product_id)].keys())[0] not in shopping_cart_dict.keys():  #编号是字符 or 编号不存在商品字典 or 编号不存在购物车里
            print ("输入错误，购物车字典没有这个商品或者商品字典没有这个商品编号，请重新输入！\n")
            continue   #重新输入吧！
        elif str.isalnum(product_id): #编号是数字
            while True:  #第二层循环
                mody_shop_num=input("请输入修改%s的数量:(注意0是代表删除哦):\n" %(Item[int(product_id)].keys()[0]))
                ...
                if str.isalpha(mody_shop_num):   #判断输入修改数量的是否字符，是字符就continue，重新输入吧！
                    ...
                    surplus_balance=surplus_balance + shopping_cart_dict[list(Item[int(product_id)].keys())[0]][0] * shopping_cart_dict[list(Item[int(product_id)].keys())[0]][1] #重新计算余额，删除的这个商品的金额，要退回去我的余额里呢
                    del shopping_cart_dict[list(Item[int(product_id)].keys())[0]]  #删除购物车字典里的商品！
                    break  #退出第二层循环

                elif mody_shop_num != "0":      #如果输入的不是字符也不是数字0，那就是修改商品的数量>0了！
                    if int(mody_shop_num) == shopping_cart_dict[list(Item[int(product_id)].keys())[0]][1]:  #修改商品的数量=购物车的数量
                        print ("修改商品的数量等于购物车的数量啦！！没什么用啊！你妹的，修改别的啊！\n")
                        break  #退出第二层循环
                    elif int(mody_shop_num) < shopping_cart_dict[list(Item[int(product_id)].keys())[0]][1]:  #修改商品数量<购物车数量
                        retrogress_num=shopping_cart_dict[list(Item[int(product_id)].keys())[0]][1] - int(mody_shop_num)  #删除这个商品的数量
                        surplus_balance = surplus_balance + shopping_cart_dict[list(Item[int(product_id)].keys())[0]][0] * retrogress_num #删除这个商品数量的金额要回退到我的余额里呢
                        shopping_cart_dict[list(Item[int(product_id)].keys())[0]][1] = int(mody_shop_num)  #修改这个商品在购物字典里的数量
                        break  #退出第二层循环

                    elif int(mody_shop_num) > shopping_cart_dict[list(Item[int(product_id)].keys())[0]][1]: #修改商品数量>购物车数量
                        increase_num=int(mody_shop_num) - shopping_cart_dict[list(Item[int(product_id)].keys())[0]][1] #计算增加了多少个商品
                        increase_shop_balance = shopping_cart_dict[list(Item[int(product_id)].keys())[0]][0] * increase_num #计算增加这几个商品的金额
